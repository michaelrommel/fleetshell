<!--
	Service catalog tree (left column of /services/catalog). Mirrors
	ProductTree.svelte: loads the whole category/service tree (parent_id derived
	from the ltree path) and renders it with client-side expand/collapse + a
	filter that reveals matches with their ancestors. Selection is a link, so the
	host page owns the URL via hrefFor(id).
-->
<script lang="ts">
...
<input class="search" placeholder="Filter services" bind:value={filter} />
<div class="card tree">
...
<style>
	...
	.kind-root { color: var(--text-muted); }
