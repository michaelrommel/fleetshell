<!--
	Product tree (left column of /products). Loads the whole modality/product/model
	tree (parent_id derived from ltree path) and renders it with client-side
	expand/collapse + a filter that reveals matches with their ancestors.
	Selection is a link, so the host page controls the URL via hrefFor(id).
	Mirrors GroupTree.svelte; kept separate because the node shape differs (kind
	badge instead of grant/member counts).
-->
<script lang="ts">
	import { untrack } from 'svelte';

	type Node = { id: string; name: string; parent_id: string | null; kind: string; child_count: number };
	let {
		nodes,
		selectedId = null,
		hrefFor,
	...
	const visibleIds = $derived.by(() => {
		...
		for (const n of nodes) {
			if (n.name.toLowerCase().includes(f)) {
	...
	$effect(() => {
		...
		const _n = nodes;
		untrack(() => {
			// Collapsed by default (overview of modalities); only reveal the path
...
<input class="search" placeholder="Filter products" bind:value={filter} />
<div class="card tree">
...
<style>
	...
	.kind-model { color: var(--text-muted); }
