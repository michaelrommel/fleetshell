...
export const actions: Actions = {
	...
	createNode: async ({ request, locals }) => {
		...
		throw redirect(303, `${base}/products?sel=${encodeURIComponent(row.id)}`);
	},
	...
	rename: async ({ request, locals }) => {
		...
		throw redirect(303, `${base}/products?sel=${encodeURIComponent(id)}`);
	},
	...
	deleteNode: async ({ request, locals }) => {
		...
		if (!node) throw redirect(303, `${base}/products`);
		...
		throw redirect(303, `${base}/products`);
	},
	...
	saveApps: async ({ request, locals }) => {
		...
		throw redirect(303, `${base}/products?sel=${encodeURIComponent(productId)}`);
	},
};
