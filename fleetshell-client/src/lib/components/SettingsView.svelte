<script lang="ts">
  ...
  let cfg: AppConfig = $state({ font_size: FONT_DEFAULT, vnc_viewer: '' });

  // Separate save-state indicators per setting so they don't interfere.
  ...
  async function saveConfig(indicator: 'font' | 'vnc') {
    const setState = indicator === 'font'
      ? (s: typeof fontSaveState) => { fontSaveState = s; }
      : (s: typeof vncSaveState)  => { vncSaveState  = s; };
...
<style>
  ...
  .path-input:focus { outline: 1px solid var(--yellow); border-color: var(--yellow); }
